...
@DisplayName("NanoIdGeneratorTool.generate")
class NanoIdGeneratorToolTest {

    @Test
    @DisplayName("generated ID has requested length")
    void correctLength() {
        assertThat(NanoIdGeneratorTool.generate(10)).hasSize(10);
        assertThat(NanoIdGeneratorTool.generate(21)).hasSize(21);
        assertThat(NanoIdGeneratorTool.generate(50)).hasSize(50);
    }
    ...
    void onlyAlphabetCharacters() {
        ...
        for (char c : NanoIdGeneratorTool.alphabet()) alphabet.add(c);

        for (int i = 0; i < 20; i++) {
            String id = NanoIdGeneratorTool.generate(100);
    ...
    void alphabetSize() {
        assertThat(NanoIdGeneratorTool.alphabet()).hasSize(64);
    }

    @Test
    @DisplayName("alphabet contains URL-safe characters only")
    void alphabetUrlSafe() {
        for (char c : NanoIdGeneratorTool.alphabet()) {
    ...
    void uniqueAcrossCalls(int length) {
        ...
        for (int i = 0; i < 50; i++) {
            String id = NanoIdGeneratorTool.generate(length);
    ...
    void minimumLength() {
        String id = NanoIdGeneratorTool.generate(1);
